...
const TEAMS = [
  ...
  { id:12, name:"Senegal",      code:"YOU", flag:"🇸🇳", group:"C", color:"#00853F", jersey:"#00853F" },
...
// WEBSOCKET PVP HOOK
// ============================================================
const BACKEND_URL = process.env.REACT_APP_BACKEND_URL || "wss://wcuppenalty-production.up.railway.app";
...
function PVPTab({ selectedTeam, onSelectTeam, onStartPVPMatch }) {
  ...
  const pvp = usePVPSocket({
    onOpponentJoined: (opp) => {
      setOpponent(opp);
      setPvpPhase("payment");
      addLog(`${opp.team?.flag} Opponent found! Processing payment...`);
    },
    onPaymentConfirmed: () => {
      setPvpPhase("ready");
      addLog("✅ Ödeme in Phantomndı! Maç başlıyor...");
    },
    onMatchStart: (data) => {
      setPvpPhase("playing");
    ...
    onMatchOver: (data) => {
      ...
      if (won) {
        ...
        addLog(`🏆 YOU WIN! +${WIN_AMT} ${currency} sent!`);
      } else {
        addLog(`😢 You lost. Try again!`);
      }
    },
  });
  ...
  // Fetch open rooms
  useEffect(() => {
    fetch("https://wcuppenalty-production.up.railway.app/rooms")
      .then(r => r.json())
      .then(setOpenRooms)
      .catch(() => setOpenRooms([]));
  ...
  function handleCreateRoom() {
    ...
    setTotalFees(f => f + FEE_AMT);
    setCoinBal(c => c + FEE_AMT * 10);
    addLog(`Room created · ${ENTRY_FEE} ${currency} bet`);
  }
  ...
  function handleJoinRoom(room) {
    ...
    setPvpPhase("payment");
    addLog(`${room.team?.flag} joined room`);
  }
    ...
    <div style={{ padding:"14px" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize:"12px", color:"#bbb", lineHeight:"1.85" }}>
          ...
          • Real 2 players · WebSocket · Auto payout
        </div>
      </div>
          ...
          {!selectedTeam ? (
            ...
            <div>
              ...
              <button onClick={handleCreateRoom} disabled={!wallet || balance < ENTRY_FEE} style={{
                ...
                💰 CREATE ROOM ({ENTRY_FEE} {currency})
              </button>
      ...
      {pvpPhase === "waiting" && (
        <div style={{
          ...
          <div style={{ fontSize:"16px", color:"#FFD700", fontWeight:"700", marginTop:"10px" }}>Waiting for opponent...</div>
          <div style={{ fontSize:"12px", color:"#888", marginTop:"4px" }}>
            Room: <strong style={{color:"#00FF88"}}>#{pvp.roomId}</strong>
          </div>
          <div style={{ fontSize:"11px", color:"#666", marginTop:"8px" }}>Share with your friend!</div>
      ...
      {pvpPhase === "payment" && (
        <div style={{
          ...
          <div style={{ fontSize:"32px", marginBottom:"10px" }}>💳</div>
          <div style={{ fontSize:"15px", color:"#FFD700", fontWeight:"700", marginBottom:"6px" }}>Payment Required</div>
          <div style={{ fontSize:"12px", color:"#aaa", marginBottom:"14px" }}>
            Opponent found! Confirm <strong style={{color:"#FFD700"}}>{ENTRY_FEE} {currency}</strong> in Phantom
          </div>
          {opponent && (
      ...
      {pvpPhase === "result" && matchResult && (
        <div style={{
          ...
          <div style={{
            ...
            {matchResult.winner === pvp.playerId ? "YOU WIN!" : "YOU LOST"}
          </div>
      ...
      <div style={{
        ...
        Every {FEE_AMT} {currency} fee → pump.fun'da <strong style={{color:"#FFD700"}}>WCUP</strong> otomatik satın alır.
        <div style={{ marginTop:"5px", color:"#555", fontSize:"10px" }}>⚠️ Mainnet · Real money</div>
      </div>
    </div>
  );
}
  ...
  async function refresh() {
    ...
    try {
      const res = await fetch(
        `https://wcuppenalty-production.up.railway.app/leaderboard?period=${period}&limit=10`
      );
      if (res.ok) {
    ...
    <div style={{ padding:"14px" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize:"11px", color:"#888", marginTop:"4px" }}>
          ...
          {" · "}
          <span style={{ color:"#FF6600" }}>{daysLeft} days left</span>
        </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{textAlign:"center"}}>EARNINGS</span>
      ...
      <div style={{
        ...
        🕐 Weekly leaderboard resets every <strong style={{color:"#FFD700"}}>Monday 00:00</strong> <br/>
        Top 3 earn special <strong style={{color:"#FF6600"}}>WCUP token ödülü</strong> kazanır
      </div>
    </div>
  );
}
